...
function IconButton({ href, label, children, size }: { href: string; label: string; children: ReactNode; size: 'sm' | 'md' }) {
  ...
  return (
    <a
      ...
      aria-label={label}
      className={`${dim} inline-flex items-center justify-center rounded-full border border-zinc-700 bg-zinc-900/80 text-zinc-300 hover:text-white hover:border-zinc-500 hover:bg-zinc-800 transition-colors`}
    >
      <span style={{ width: icon, height: icon }} className="flex items-center justify-center">{children}</span>
    </a>
  );
}
...
export default function SocialLinks({ whatsapp, instagram, facebook, email, size = 'md', className = '' }: Props) {
  ...
  return (
    <div className={`flex flex-wrap gap-3 ${className}`}>
      {links.map(l => (
        <IconButton key={l.label} href={l.href} label={l.label} size={size}>
          {l.icon}
        </IconButton>
      ))}
    </div>
